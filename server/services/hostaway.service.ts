...
export function normalizeHostawayItem(item: any) {
...
export async function syncHostawayReviews() {
  ...
  const ops = normalized.map((r) =>
    Review.findOneAndUpdate({ reviewId: r.reviewId }, r, { upsert: true, new: true, setDefaultsOnInsert: true }).exec()
  );
...
export async function getReviews(filters?: {
  ...
  const query: any = {};
  if (filters?.listing) query.listingName = { $regex: filters.listing, $options: "i" };
  if (typeof filters?.approved === "boolean") query.approved = filters.approved;
  if (filters?.date_from || filters?.date_to) {
    query.submittedAt = {};
    if (filters.date_from) query.submittedAt.$gte = new Date(filters.date_from);
    if (filters.date_to) query.submittedAt.$lte = new Date(filters.date_to);
  }
  if (typeof filters?.rating_min === "number") query.ratingOverall = { $gte: filters.rating_min };
